...
headerline1.setAttribute("href", futureHeadlineLink + 
`&utm_source=nb-blog-${dagWeek}&utm_medium=email&utm_campaign=headline&utm_content=%7c${sendDate}%7cheadline%7c`)
...
function artikelenGrootItems(item, index) {
  ...
  var item_categorie = item_categorie + '<span class="postScore">&#9733 '+popularityscore+'</span><span class="w100"></span>'
...
function functionVacatureItems(item, index) {
  ...
  var vac_link = item.querySelector("link").innerHTML + `?utm_source=al-jobs-${dagWeek}&amputm_medium=email&amputm_campaign=vacature&amputm_content=%7c${sendDate}%7cvacature%7c`
  if(dagWeek != 'dagelijks') {
    var vac_link = item.querySelector("link").innerHTML + `?utm_source=nb-jobs-${dagWeek}&amputm_medium=email&amputm_campaign=vacature&amputm_content=%7c${sendDate}%7cvacature%7c`
  }
